...
unsigned short endianSwap(unsigned char one, unsigned char two)
{
    // This is stub code!
    // return 0x0000;
    short msb = two; //most significant bit 
    msb = msb << 8; //move msb to left to make big endian
    short lsb = one; //least significant bit
    short ret = msb | lsb; //or to get return short
    return ret;
}
