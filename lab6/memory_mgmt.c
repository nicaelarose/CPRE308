...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int leastTimeOfArrival = PageFrames[0].time_of_arrival;
	int i;

	for(i = 1; i < num_frames; i++) //finding the least time of arrival
	{
		if(PageFrames[i].time_of_arrival < leastTimeOfArrival)
		{
			leastTimeOfArrival = PageFrames[i].time_of_arrival;
			frame_to_evict = i;
		}
	}
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0;
	/*TODO: fill in the code below */
	int leastAccessTime = PageFrames[0].time_of_access;
	int i;

	for (i = 1; i < num_frames; i++) //find the least access time
	{
		if (PageFrames[i].time_of_access < leastAccessTime)
		{
			leastAccessTime = PageFrames[i].time_of_access;
			frame_to_evict = i;
		}
	}
	return frame_to_evict;
...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	for (i = 0; i < num_frames; i++) //not found yet so setting to 0 here
	{
		found[i] = 0;
	}
	...
	{
		finished = 1;
		for (j = 0; j < num_frames; j++) //page frame is not processed
		{
			if (found[j] == 0)
			{
				finished = 0;
		...
		if (finished) //once alll page frames have been processed
		{
			break;
		}
	}
